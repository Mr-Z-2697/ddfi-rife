...
x265_default='-c:v libx265 -preset 6 -x265-params sao=0:rect=0:strong-intra-smoothing=0:open-gop=0:b-intra=1:weightb=1:aq-mode=1:aq-strength=0.8:me=hex:subme=2:info=0'
...
offs1.set_output()''' % (threads,args.maxmem,tmpV)
    ...
    scd=''
    interp='''clip=core.std.Expr(clip,'x 0.7 * 0.15 +')
clip=core.trt.Model(clip,r'D:\Misc\cugan-ps2-trt-512x512.engine')
...
%s
clip = core.resize.Bicubic(clip,format=vs.RGB48,matrix_in=1).fmtc.bitdepth(bits=32)
%s
clip = core.resize.Bicubic(clip,format=vs.YUV420P10,matrix=1,dither_type='error_diffusion')
clip = core.vfrtocfr.VFRToCFR(clip,r"tsv2o.txt",24000,1001,True)
clip.set_output()
''' % (threads,args.maxmem,tmpV,scd,interp)
