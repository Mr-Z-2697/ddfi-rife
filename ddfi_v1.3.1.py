...
class args:
    pass
parser = argparse.ArgumentParser(description='anime auto duplicated frame remove and frame interpolate tool',formatter_class=argparse.RawTextHelpFormatter)
...
parser.add_argument('-q','--output_crf',required=False,type=str,help='output video crf value, default 18')
parser.add_argument('-qi','--intermedia_crf',required=False,type=str,help='intermedia video crf value, default 12')
...
tmpInterpX8=tmpFolder+'interpX8_done.264'
tmpTSV2O=tmpFolder+'tsv2o.txt'
tmpTSV2N=tmpFolder+'tsv2n.txt'
...
if not os.path.exists(tmpInterpX8):
...
smoothfps_params="{rate:{num:16,den:2},algo:23,cubic:1}"
super = SVSuper(super_params)
vectors = SVAnalyse(super, analyse_params)
SVSmoothFps(super, vectors, smoothfps_params, mt=threads)
Prefetch(threads)
'''
    avs=open(tmpFolder+'interpX8.avs','w')
    print(script,file=avs)
    avs.close()
    ff_interp=f'\"{ffpath}ffmpeg\" -i \"{tmpFolder}interpX8.avs\" -crf {crfi} -preset 1 -pix_fmt yuv420p -c:v libx264 -map v:0 \"{tmpFolder}interp.264\" -y'
    print(ff_interp)
    subprocess.run(ff_interp,shell=True)
    os.rename(f'{tmpFolder}interp.264',tmpInterpX8)

if not os.path.exists(f'{tmpInterpX8}.mkv'):
    ...
    mmg=f'\"{mmgpath}mkvmerge.exe\" -o \"{tmpInterpX8}.mkv\" --timestamps 0:\"{tmpTSV2N}\" \"{tmpInterpX8}\"'
    print(mmg)
    subprocess.run(mmg,shell=True)

ff_down60=f'\"{ffpath}ffmpeg.exe\" -i \"{tmpInterpX8}.mkv\" -i {tmpDedup} -map 0:v:0 {ffau2} -vf framerate=60000/1001 -crf {crfo} -pix_fmt yuv420p -c:v libx264 -c:a libopus -ac 2 \"{outFile}\" -y'
print(ff_down60)
